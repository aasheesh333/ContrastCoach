...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      // Guard: Firebase may not be initialized (offline / config missing).
      // Home does the same tryGet() bail-out; without it Settings hangs
      // on a spinner forever because _load throws before setState.
      if (FirebaseAuthNullableProxy.tryGet() != null) {
        ...
        if (profileResult is Ok<UserProfile, AppException>) {
      ...
      try {
        ...
        // DB open failed — render empty state rather than spin forever.
      }
    } catch (_) {
      // Any unexpected failure — still show the screen.
    } finally {
      if (mounted) {
        setState(() {
          _profile = profile;
